...
class Caseclasses extends FlatSpec with Matchers{
  ...
  "Case classes" should "implement eq/equals method" in {
    ...
    d3.eq(d4) shouldBe true

    // equals method compares if they are equivalent

    d1.equals(d3) shouldBe true
  }

  it should "have a convenient hashCode method that works" in {
  ...
  it should "have automatic properties" in {
    ...
    d1.name shouldBe "Scooby"
    d1.breed shouldBe "Doberman"
  ...
  it should "have safer alternatives for altering" in {
    ...
    // d4.name = "Mumu" WE CAN'T ALTER A val

    d4.name should be("Leo")

    val d5 = d4.copy(name = "Mumu") // The alternative is create a copy with the change
